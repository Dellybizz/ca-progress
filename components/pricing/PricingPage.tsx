    ...
    try {
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (result: CheckoutResponse) => {
          ...
          if (!verify.ok) {
            setNotice(verified.error || "Payment verification failed");
            return;
          }
        ...
        notes: { user_id: session.user.id, plan_id: plan.id },
  ...
  return (
    <main className="pricing-shell">
      <header>
        ...
        <button
          onClick={() =>
            session ? (window.location.href = "/settings") : requireAuth()
          }
        >
      ...
      <section className="pricing-hero">
        ...
        <h1>Choose the tools that match your preparation</h1>
        <p>
          Upgrade securely and unlock eligible sections immediately. Plans renew
          manually each month.
        </p>
      </section>
      {authLoading ? (
        <p className="pricing-loading">Loading account…</p>
      ) : (
        <section className="pricing-grid">
          {plans.map((plan) => (
            <article
              key={plan.id}
              className={plan.slug === "pro" ? "featured" : ""}
            >
              {plan.slug === "pro" && (
              ...
              <ul>
                ...
                <li>
                  <Check />
                  {plan.rank > 0
                    ? "Premium sections based on plan"
                    : "Core free sections"}
                </li>
                <li>
                  <ShieldCheck />
              ...
              <button
                disabled={busy === plan.id || planSlug === plan.slug}

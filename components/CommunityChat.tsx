...
export default function CommunityChat({
  ...
  return (
    <section className={`chat-shell${mobileChatOpen ? " channel-open" : ""}`}>
      <aside className="chat-sidebar">
        ...
        <nav className="chat-channels" aria-label="Community channels">
          ...
          {doubtChannels.map(([id, label, description]) => (
            <button
              type="button"
              key={id}
              className={id === activeChannel ? "active" : ""}
      ...
      <style>{`
        ...
        .chat-sidebar{min-width:0;min-height:0;overflow:hidden;border-right:1px solid #e9edf3;background:#f9fbfe;display:flex;flex-direction:column}.chat-brand{height:88px;flex:0 0 88px;padding:20px;display:flex;align-items:center;gap:11px;border-bottom:1px solid #e9edf3;color:#26364c}.chat-brand div{display:grid;gap:3px}.chat-brand strong{font-size:14px}.chat-brand span{font-size:10px;color:#8a96a8}
        .chat-channels{flex:1;min-height:0;overflow-y:auto;padding:12px 10px;display:grid;align-content:start;gap:3px;overscroll-behavior:contain}.channel-group-title{padding:4px 10px 6px;color:#99a3b2;font-size:8px;font-weight:800;letter-spacing:.8px;text-transform:uppercase}.channel-group-title.study-title{margin-top:10px}.chat-channels button{width:100%;min-width:0;height:39px;padding:0 10px;display:flex;align-items:center;gap:8px;border:0;border-radius:9px;background:transparent;color:#6f7d91;text-align:left;font-size:11px}.chat-channels button:hover{background:#eef3f9}.chat-channels button.active{background:#eaf1ff;color:#2863c7;font-weight:700}.channel-copy{min-width:0;display:block;overflow:hidden}.channel-copy b{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}.channel-copy small{display:none}
